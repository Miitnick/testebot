...
// FUNCTION
	...
	// Filter Banned People
            ...
            case 'ping':
                ...
                case 'setpic':
                    ...
                    if (isMedia) {
                        ...
                        aruga.sendTextWithMentions(`Obrigado @${sender.id.replace('@c.us','')} Foto de perfil..`)
                        ...
                        aruga.reply(from, `Formato errado!\n⚠️ Envie a imagem com ${prefix}setpic`, id)
                    }
                    break
        case 'getpic':
            ...
            const useriq = getnomber.id.replace('@','') + '@c.us'
                try {
                    ...
                    aruga.sendFileFromUrl(from, jnck, `awok.jpg` , `Aqui está`)
                } catch {
        ...
        case 'tnc':
            ...
            break
        case 'help':
            const bots = `Olá, aqui é Rorschach Bot, para descobrir o menu de comandos, digite *${prefix}menu*, *${prefix}p*`
    ...
    if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa digunakan didalam grup!', id)
            fetch('https://raw.githubusercontent.com/Miitnick/randons/main/Verdade.txt')
            .then(res => res.text())
            .then(body => {
            ...
            .catch(() => {
                aruga.reply(from, 'Oops, algo deu errado !!', id)
            })
            break
    case 'dare':
    if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa digunakan didalam grup!', id)
            fetch('https://raw.githubusercontent.com/Miitnick/randons/main/Desafio.txt')
            .then(res => res.text())
            .then(body => {
     ...
     if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
            ...
            aruga.reply(from, 'Algo deu errado!', id)
             ...
             break
         case 'kbbi':
            if (args.length == 0) return aruga.reply(from, `Para pesquisar uma palavra no Grande Dicionário Indonésio (KBBI) \ntipo: ${prefixo}kbbi [palavra]`, id)
            const kbbip = body.slice(6)
            const kbbis = await rugaapi.kbbi(kbbip)
            await aruga.reply(from, kbbis, id)
            .catch(() => {
                aruga.reply(from, 'Oops, há um erro !!', id)
            })
            break
            case 'blackpink':
                if (args.length == 0) return aruga.reply(from, `enviar comando ${prefix}logoff [nome]`, id)
                aruga.reply(from, mess.wait, id)
            ...
            case 'logoff':
                ...
                break
        case 'pornhub':
            if (args.length === 1) return aruga.reply(from, `Enviar comando *${prefix} pornhub [| Texto1 | Texto2]*,\n\n exemplo: *${prefix} pornhub | Dimas | CUBO*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                aruga.reply(from, `paciencia processo lento ....`, id)
                const lpornhub = argz[1]
                const lpornhub2 = argz[2]   
                if (lpornhub > 10) return aruga.reply(from, '*Teks1 Muito longo!*\nMáximo 10 letras!_', id)
                ...
                await aruga.reply(from, `Formato errado!\n[❗] Enviar comando *${prefix}pornhub [| Texto1 | Texto2]*,\n\nExemplo: *${prefix}logopornhub | Dimas | CUBO*`, id)
            }
            break
        case 'slightning':
        ...
        case 'stickerfire':
            if (isMedia && type === 'image') {
                ...
                await aruga.reply(from, `Formato errado!\n⚠️ Envie a imagem com ${prefix}stickerfire`, id)
            }
            break
        case 'thunder':
            if (args.length === 1) return aruga.reply(from, `Enviar comando *${prefix}thunder [Texto]*, por exemplo *${prefix}thunder Tobz*`, id)
            aruga.reply(from, mess.wait, id)
        ...
        case 'tebakgambar':
            ...
            const jwban = `➸ Responda : ${resp.data.result.jawaban}`
            ...
            aruga.reply(from, jwban, id)
                ...
                await aruga.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Desculpe, as perguntas do questionário não foram encontradas')
           }
           break
        case 'caklontong':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
            ...
            if (resp.data.error) return aruga.reply(from, resp.data.error, id)
            const anm2 = `➸ Pergunta : ${resp.data.result.soal}\n\n➸ Poin : ${resp.data.result.poin}`
            const jwban = `➸ Resposta : ${resp.data.result.jawaban}\n\n➸ Deskripsi : ${resp.data.result.desk}`
	...
	case 'owner':
            ...
            .then(() => aruga.sendText(from, 'Não faça muitas perguntas, este não é StackOverFlow!'))
            break
            case 'maps':
			...
			aruga.sendStickerfromUrlAsReply(from, res.data.url, '', '', id)
                ...
                break
                case 'boobs':
                if (!isPrem) return aruga.reply(from, 'Comando disponivel apenas para usuarios Premiums\n Contate o dono do bot', id)
                aruga.reply(from, mess.wait, id);
		...
		break
            ...
            case 'baka':
                ...
                case 'aesthetic':
                    ...
                    await aruga.sendImage(from, anjayani, 'img.jpg', 'wallpaper', id)
                    break
                case 'pictcogan':
                ...
                case 'antilink':
                    ...
                    if (args[0] == 'on') {
                        ...
                        if(cek){
                            ...
                            aruga.reply(from, '*[Anti Link de Grupo]* foi ativado\nCada membro do grupo que enviar uma mensagem contendo um link de grupo será expulso pelo bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(!cek){
		...
		case 'invite':
            ...
            break
                    ...
                    const caption = `*Detalhes do membro*✨ \n\n● *Nome:* ${name}\n● *Biografia:* ${bio.status}\n● *Link do bate-papo:* wa.me/$(sender.id.replace('@c.us', ")}\n● *Função:* ${adm}\n● *Banido pelo Bot:* ${ban ? '✔': '❌'}\n● *Bloqueado pelo Bot:* ${isblocked ? '✔': '❌'}\n● *Converse com bot:* ${isOnline}`
                ...
                case 'jadian':
                    ...
                    const sapa = `Ola... @${aku.replace(/[@c.us]/g, '')} (💘) @${kamu.replace(/[@c.us]/g, '')} acabou de acontecer aqui\nPara o pj por favor`
                    await aruga.sendTextWithMentions(from, sapa)
                    break     
            ...
            case 'bkp': // MFARELS
                ...
                break
        case 'join':
            if (args.length == 0) return aruga.reply(from, `Se você quiser convidar o bot para um grupo, convide ou digite ${prefix} para entrar no [link do grupo]`, id)
		if (!isOwnerB) return aruga.reply(from, `Proprietário do bate-papo para entrar`, id)
            ...
            if (isOwnerB) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `E aí vocês, eu sou o Rorschach Bot. Para descobrir os comandos deste Bot digite menu ${prefix}menu`)
                      })
            } else {
            ...
            break
        case 'wattpadstory':
            if (args.length == 0) return aruga.reply(from, `Para pesquisar histórias no wattpad! Usar ${prefix}wattpadstory url story\nExemplos : ${prefix}wattpadstory https://www.wattpad.com/story/226120582-my-teacher-levi-x-student-reader`, id)
            ...
            try {
                ...
                let wtpdst =  `*「 WATTPAD 」*\n\n*Resultado da pesquisa : ${wpstry}*\n`
                for (let i = 0; i < datplay.parts.length; i++) {
                    wtpdst += `\n─────────────────\n\n*A U T H O R :* ${datplay.author.name}\n\n• *Título :* ${datplay.title}\n• *Ser lido :* ${datplay.reads}\n• *Votos :* ${datplay.votes}\n• *Número de episódios :* ${datplay.parts_count}\n• *Descrição :* ${datplay.desc}\n\n• *Título :* ${datplay[i].title}\n• *URL :* ${datplay[i].url}\n`
                }
        ...
        case 'shopee':
            if (!isGroupMsg) return aruga.reply(from, `Este comando só pode ser usado em grupos!`, id)
            if (args.length === 1) return aruga.reply(from, `Enviar pedidos *${prefix}shopee [ Consulta ]*, Exemplo : *${prefix}shopee HP Samsul a20*`)
        ...
        case 'playstore':
            if (!isGroupMsg) return aruga.reply(from, `Este comando só pode ser usado em grupos!`, id)
            if (args.length === 0) return aruga.reply(from, `Enviar pedidos *${prefix}playstore [ Consulta ]*, Exemplo : *${prefix}playstore Mobile Legends*`)
	...
	case 'reportbug':
		...
		await aruga.sendText(ownerNumber, `Relatório de bug de : *${pushname}*\nNomor : ${serial.replace('@c.us', '')}\n\nBug : *${reporter}*`)
		aruga.reply(from, 'Relatório enviado com sucesso ao Owner Bot!', id)
		break
        case 'setgroupname':
            if (!isGroupMsg) return aruga.reply(from, `Este recurso só pode ser usado em grupos`, id)
        ...
        case 'setname':
                ...
                case 'read':
                    if (!isGroupMsg) return aruga.reply(from, `Este comando só pode ser usado em grupos!`, id)                
                    if (!quotedMsg) return aruga.reply(from, `Marque a mensagem do bot`, id)
                    if (!quotedMsgObj.fromMe) return aruga.reply(from, `Marque a mensagem do bot`, id)
                    try {
                        ...
                        aruga.sendTextWithMentions(from, `Não se preocupe.. Venha agora\n${list}`)
                    } catch(err) {
                        console.log(err)
        ...
        case 'botstat': {
            ...
            const charged = await aruga.getIsPlugged();
            const device = await aruga.getMe(); 
            const deviceinfo = `- Batteria : ${device.battery}%\n  ├ Está carregando : ${charged}\n  └ 24 Hours Online : ${device.is24h}\n  ├ OS Version : ${device.phone.os_version}\n  └ Build Number : ${device.phone.os_build_number}\n\n\n _*Hora :*_ ${moment(t * 1000).format('HH:mm:ss')}\n`   
            const chatIds = await aruga.getAllChatIds()
            const groups = await aruga.getAllGroups()
            const groupsIn = groups.filter(x => x.groupMetadata.participants.map(x => [botNumber, '62821992491007@c.us'].includes(x.id._serialized)).includes(true))
	...
	case 'cooltext':
		...
		break
	case 'raingif':
		if (args.length == 0) return aruga.reply(from, `Para fazer adesivos GIF de chuva, use ${prefix}raingif [url]\n\nExemplo : ${prefix}raingif https://avatars.githubusercontent.com/Urbaee`, id)
		...
		break
	case 'triggered':
		if (args.length == 0) return aruga.reply(from, `Para criar um GIF acionado \ nUse ${prefix}triggered link foto\nExemplo : ${prefix}trigggered https://avatars.githubusercontent.com/Urbaee`, id)
		aruga.reply(from, mess.wait, id)
		...
		break
	case 'kisahnabi':
		if (args.length == 0) return aruga.reply(from, `Enviar pedidos ${prefix}história do nome do profeta\nExemplos : ${prefix}kisahnabi adam`, id)
		...
		.catch((err) => {
			aruga.reply(from, 'Desculpe, o nome do profeta que você digitou está errado', id)
		})
		})
		.catch((err) => {
		aruga.reply(from, 'Desculpe, o nome do profeta que você digitou está errado', id)
		...
		break
	case 'tr':
		if (args.length == 0) return aruga.reply(from, `Enviar pedidos ${prefix}tr [código de linguagem] [reply caption]\n\nExemplo : ${prefix}tr id [reply caption}`, id)
	...
	if ((isMedia || isQuotedImage) && args.length === 0) {
		    ...
		    aruga.reply(from, 'O formato da mensagem está errado...', id)
		}
			break
				case 'movie':
				if (args.length == 0) return aruga.reply(from, `Para pesquisar um filme em um site pirata:v\n${prefix}movie o contador misterioso`, id)
                ...
                break
                case 'wattpad':
                    if (args.length == 0) return aruga.reply(from, `Para buscar um detalhe na seção de história do Wattpad! Usar ${prefix}wattpad [Consulta]\nExemplos : ${prefix}wattpad chefe da luxúria`, id)
                    await aruga.reply(from, mess.wait, id)
                    rugaapi.wp(args)
                    .then(async ({ result }) => {
                        let watpd = '*-----「 WATTPAD 」-----*'
                        for (let i = 0; i < result.length; i++) {
                            watpd += `\n\n• *Título :* ${result[i].title}\n• *Ser lido :* ${result[i].reads}\n• *Votos :* ${result[i].votes}\n\n• *Descrição :* ${result[i].description}\n• *URL :* ${result[i].url}`
                        }
                    ...
                    break
                case 'neonime':
                        if (args.length == 0) return aruga.reply(from, `Procure por anime no site Neonime!\nExemplo: ${prefix}neonime boruto`, id)
                        await aruga.reply(from, mess.wait, id)
                        rugaapi.neo(body.slice(9))
                        .then(async ({ result }) => {
                            let neoni = '*-----「 NEONIME 」-----*'
                            for (let i = 0; i < result.length; i++){
                                neoni +=`\n\n• *Titulo :* ${result[i].title}\n• *Url :* ${result[i].url}\n• *Descrição :* ${result[i].desc}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                            }
                        ...
                        break
                    case 'filmapik':
                        if (args.length == 0) return aruga.reply(from, `Compre pelo anime no site Neonime!\nExemplo: ${prefix}filmapik Revolutionary Love`, id)
                        await aruga.reply(from, mess.wait, id)
                        ...
                        break
	case 'brainly':
            if (args.length == 0) return aruga.reply(from, `Procurando uma resposta no site da Brainly!\nExemplos: ${prefix} exemplos de inteiros`, id)
	...
	case 'sgif':
	  aruga.reply(from, mess.wait, id)
           if (isMedia && type === 'video' || mimetype === 'image/gif') {
                try {
                    const mediaData = await decryptMedia(message, uaOverride)
                    await aruga.sendMp4AsSticker(from, mediaData, {crop: true, fps: 30, startTime: `00:00:00.0`, endTime : `00:00:10.0`,loop: 0}, {keepScale: true})
                } catch (err) {
                    aruga.reply(from, `O tamanho da mídia é muito grande! por favor, reduza a duração do vídeo.`, id)
                }
            } else if (quotedMsg && quotedMsg.type == 'video' || quotedMsg && quotedMsg.mimetype == 'image/gif') {
                const mediaData = await decryptMedia(quotedMsg, uaOverride)
                await aruga.sendMp4AsSticker(from, mediaData, {crop: true, fps: 30, startTime: `00:00:00.0`, endTime : `00:00:10.0`,loop: 0}, {keepScale: true})
            } else {
                aruga.reply(from, `Erro ⚠️ Só pode ser vídeo/gif se o arquivo de mídia estiver na forma de uma imagem digitada /stickergif`, id)
		.catch((err) => {
			aruga.reply(from, `Erro! O tamanho da mídia é muito grande! 5 segundos no máximo!`, id)
		})
    	}
            break
        case 'stikergiphy':
...
break
case 'detail':
    if (args.length == 0) return aruga.reply(from, `Para pesquisar detalhes de uma área!\nExemplo : ${prefix}detail Jakarta`, id)
...
break
    ...
    case 'linkgc':
            ...
            if (isGroupMsg) {
                ...
                aruga.sendLinkWithAutoPreview(from, inviteLink, `\nLink do grupo *${name}* Usar *${prefix}revoke* para revogar o link`)
            } else {
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Este comando só pode ser usado quando o bot se torna administrador', id)
		...
		case 'randomquran':
			...
			break
                    ...
                    break
		    case 'linknobg':
			   if (args.length == 0) return aruga.reply(from, 'Envie um link para fazer um adesivo Nobg', id)
                    ...
                    case 'imagetourl':
                        case 'imgtourl':
                            if (isMedia && isImage || isQuotedImage) {
                                ...
                                await aruga.reply(from, 'O formato está errado... envie uma imagem com  ${prefix}imgtourl', id)
                            }
                        break
                    case 'findsticker':
                        case 'findstiker':
		...
		if (!isPrem) return aruga.reply(from, 'Command Premium!\nChat owner buat mendaftar!', id)
			...
			break
                    case 'ig':
                        case 'instagram':
                           if (args.length == 0) return aruga.reply(from, `Enviar comando *${prefix}ig [linkIg]*`, id)
...
case 'ytsearch':
    ...
    case 'kusonime':
        ...
        case 'images':
            ...
            break
        case 'sreddit':
            if (args.length == 0) return aruga.reply(from, `Para procurar imagens em um sub reddit digite: ${prefix}sreddit [search]\nExemplo: ${prefix}sreddit naruto`, id)
	    if (!isPrem) return aruga.reply(from, `Comando para usuarios premiums. Contate o dono do bot!`, id)
		...
		.then(async(res) => {
            ...
            case 'facebook':
                ...
                case 'igstory':
                    ...
                    break
                case 'ig2':
                    if (args.length == 0) return aruga.reply(from, `Enviar comando ${prefix}ig2 linkig`, id)
	...
	break
            case 'twitter':
                ...
                break
		case 'tiktok':
                if (args.length == 0) return aruga.reply(from, `Enviar comando *${prefix}tiktok [linkTiktok]*`, id)
                rugaapi.toktok(args)
		.then(async(res) => {
		await aruga.sendFileFromUrl(from, res.image, '', `「 *TIKTOK* 」\n\nNome : *${res.nameInfo}*\nData de Upload : *${res.timeInfo}*\nLegenda : *${res.textInfo}*\n\n*_Waitt, Uestou enviando o vídeo..._*`, id)
	   ...
	   axios.get(`https://api.zeks.xyz/api/yts?q=${pncrian}&apikey=apivinz`)
		.then(async(res) => {
	  ...
	  console.log(color(`Resultado da pesquisa ${pncri}`, 'green'))
		...
		case 'trendingtwit':
                    case 'trendtwit':
                        ...
                        .then(async ({ result }) => {
                            ...
                            for (let i = 0; i < result.length; i++) {
                                trend += `\n\n➸ *Hashtags :* ${result[i].hastag}\n➸ *Número da tendência :* ${result[i].rank}\n➸ *Número de Tweets :* ${result[i].tweet}\n➸ *Link :* ${result[i].link}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                            }
                            await aruga.reply(from, trend, id)
                            console.log('Successo enviando Trending Tweets')
                        })
                        .catch(async (err) => {
                            console.error(err)
                            await aruga.reply(from, 'Error!', id)
	    ...
	    if (!isPrem) return aruga.reply(from, 'Command Premium\nChat owner buat mendaftar', id)
            if (args.length == 0) return aruga.reply(from, `Para pesquisar vídeos do youtube\n\nEnvie: ${prefix}play titulo do video`, id)
	    const pncri2 = body.slice(7)
            axios.get(`https://api.zeks.xyz/api/yts?q=${pncri2}&apikey=apivinz`)
            .then(async (res) => {
                await aruga.sendFileFromUrl(from, `${res.data.result[0].video.thumbnail_src}`, ``, `「 *PLAY VIDEO* 」\n\nTitulo: ${res.data.result[0].video.title}\nDuração: ${res.data.result[0].video.duration} segundos\nUpload: ${res.data.result[0].video.upload_date}\nViews: ${res.data.result[0].video.views}\nUrl: ${res.data.result[0].video.url}\n\n*_Espere, estou enviando o vídeo_*`, id)
        ...
        case 'resi':
            ...
            break
        case 'tts':
            if (args.length == 0) return aruga.reply(from, `Converte texto em audio (google voice)\nDigite: ${prefix}tts <codigo idioma> <texto>\Exemplos : ${prefix}tts id olá \ nPara saber os código do idioma, verifique aqui : https://anotepad.com/note/read/5xqahdy8`)
		...
		case 'covidindo':
			...
			break
        case 'ceklokasi':
            if (quotedMsg.type !== 'location') return aruga.reply(from, `O formato da mensagem está incorreto.\nEnvie-nos um local e responda com uma legenda ${prefix}ceklokasi`, id)
...
*Text : ${swrt}*`
	...
	case 'githubstalk':
                ...
                break
                case 'reverseword':
                    if (args.length == 0) return aruga.reply(from, `Recurso para inverter palavras\nExemplos : ${prefix}reverseword por que a letra está invertida`, id)
    ...
    case 'happymod':
        ...
        break
	case 'bot':
		if (args.length == 0) return aruga.reply(from, `Enviar pedidos ${prefix}bot [texto]\nExemplos : ${prefix}bot Olá`, id)
	...
	case 'left':
		...
		aruga.reply(from, 'Fitur left berhasil diaktifkan digrup ini!', id)
	   } else if (args[0] === 'disable') {
		left.splice(chat.id, 1)
		fs.writeFileSync('./lib/database/left.json', JSON.stringify(left))
		aruga.reply(from, 'Fitur left berhasil dinonaktifkan di grup ini!', id)
	} else {
		aruga.reply(from, 'Pilih enable atau disable', id)
	}
	break
	case 'afk':                             
              if (!isGroupMsg) return await aruga.reply(from, 'Desculpe, este recurso só pode ser usado em grupos!', id)
                if (isAfkOn) return await aruga.reply(from, `${pushname} atualmente *AFK (AWAY FROM KEYBOARD)*\n\nRazão: ${reason}`, id)
                addAfk(sender.id, time, reason)
		await aruga.sendTextWithMentions(from, `@${sender.id.replace(/@c.us/g, '')} AGORA *AFK (LONGE DO TECLADO)*\nRazão: ${reason}`)
break
	 case 'welcome':
...
Este grupo foi fundado *${date}* as *${timeh}* por @${ownerwoi.replace('@c.us','')}
...
${desc}
₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋
_Descriçao atualizado por : @${chat.groupMetadata.owner.replace('@c.us','')} no dia *${moment(chat.groupMetadata.descTime * 1000).format('DD MMMM YYYY')}* as ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
                    ...
                    case 'screen': {
                        ...
                        case 'saylist':
                            const saylest = dsay
                            let kimtil = `╔══✪〘 *Lista de falas* 〙✪══\n`
                            for (let i = 0; i < saylest.length; i++) {
                                kimtil += '╠➥'
                                kimtil += `${saylest[i]}\n`
                            }
                            kimtil += '╚═〘 *R O R S C H A C H  B O T* 〙'
                            await aruga.sendText(from, kimtil)
                            break
                        case 'addsay':{
                            if (!args.length >= 1) return aruga.reply(from, 'Qual frase é?', id)
                            const say = body.slice(8)
                                dsay.push(say)
                                fs.writeFileSync('./lib/database/say.json' , JSON.stringify(dsay))
                                aruga.reply(from, `Fala adicionada ao banco de dados\nNumero de falas : *${dsay.length - 1}* ,` , id)
                        }
                        break
                        case 'addbacot':{
                ...
                case 'say':
                    if(args.length == 1){
                        ...
                        if(sayur <= wuh) return await aruga.reply(from, `O banco de dados total é atualmente *${sayur - 1}` , id)
                        const lahs = dsay[wuh]
                ...
                case 'nyenye':
                    ...
                    case 'convertduit':
                        ...
                        break
                    case 'translate':
                        if (args.length == 0) return aruga.reply(from, `Para traduzir a palavra use ${prefix}translate [codigo idioma]| texto\n\nExemplo : ${prefix}translate pt | How are you?`, id)
			    ...
			    const texttr = `Texto : *${res.data.data.origin}*\n\Traduza para${suway1} : *${res.data.data.result}*\n\Alvo : *${res.data.data.targets[0]}*`
			...
			aruga.sendImageAsSticker(from, res.data.base64)
                 ...
                 case 'rate':
                     if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
                     if (args.length == 0) return aruga.reply(from, `Recursos para julgar o que você diz \nExemplo : ${prefix}rate minha beleza`, id)
                     const rating = args.join(' ')
                     const awr = rate[Math.floor(Math.random() * (rate.length))]
                     if (!rating) aruga.reply(from, `⚠️ Formato incorreto! Use *${prefix}menu* para ver os comandos.`)
                     await aruga.sendText(from, `Pergunta: *${rating}* \n\nResposta: ${awr}`)
                     break
                 case 'apakah':
                     if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                  ...
                  case 'bisakah':
                     ...
                     break
            case 'listban':
                let bened = `Esta é uma lista de números proibidos\nTotal : ${banned.length}\n`
                for (let i of banned) {
        ...
        case 'listblock':
            ...
            break
        case 'bc':
            if (!isOwnerB) return aruga.reply(from, `Este comando é apenas para Proprietário bot`, id)
                bctxt = body.slice(4)
                txtbc = `〘 *R O R S C H A C H  B O T* 〙\n\n${bctxt}`
        ...
        case 'howmuch':
            ...
            const yaelah = chat.groupMetadata.participants.length
                await aruga.sendText(from, `Total de membros em *${tulul}* É : *${yaelah}*` )
                break
        case 'ownergc':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
            const Owner_ = chat.groupMetadata.owner
            await aruga.sendTextWithMentions(from, `Proprietário do : @${Owner_}`)
            break
        }
